...
{	
	...
	leftTicks:RenderableTicks = null;
	bottomAxis:RenderableTicks = null;
	bottomTicks:RenderableTicks = null;
	labels = [];
	...
	constructor(Canvas, Options={}, StyleOverrides:any = {})
	{
		...
		this.leftTicks = new RenderableTicks(this.Styles.ticks);
		this.bottomAxis = new RenderableTicks(this.Styles.axes);
		this.bottomTicks = new RenderableTicks(this.Styles.ticks);

		this.leftAxis.ZIndex = -1;
		this.leftTicks.ZIndex = -1;
		this.bottomAxis.ZIndex = -1;
		this.bottomTicks.ZIndex = -1;
		this.addObject(this.leftAxis);
		this.addObject(this.leftTicks);
		this.addObject(this.bottomAxis);
		this.addObject(this.bottomTicks);
	...
	render(deltaTime)
	{
		...
		if(!this.Canvas)
			return;
		// console.log("chart render:", deltaTime);
		// Clear canvas
	...
	renderYLabels()
	{
		...
		this.Context.fillStyle = this.Styles["labels"]["font-color"];
		...
		for(var i=0;i<maxYTicks;++i)
		{
			...
			var label = rangeStep.toString().substring(0, 7);
			var textWidth = this.Context.measureText(label).width;
			this.Context.fillText(label, this.drawRect.left + this.Styles["labels"]["padding"], posY - 2);

			this.leftTicks.renderTickPoint(this.Context, {
				x: this.drawRect.left,
				y: posY
			}, {
				x: this.drawRect.right,
				y: posY
			});
		}
	...
	renderXLabels()
	{
		...
		for(var i=0;i<maxXTicks;++i)
		{
			...
			this.Context.save();		
			this.Context.translate(2 + posX + textWidth, this.drawRect.bottom);
			...
			xC += textWidth;
			this.bottomTicks.renderTickPoint(this.Context, {
				x: posX,
				y: this.drawRect.bottom
			}, {
				x: posX,
				y: this.drawRect.top
			});
